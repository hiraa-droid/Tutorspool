...
export default function StudentDashboardScreen() {
  ...
  // ────────────────────────────────────────────────
  // Same AI suggestion logic as web (local fallback only here for simplicity)
  const generateAISuggestions = (goalsList: LearningGoal[]) => {
    setAiLoading(true);

    // You can keep the Supabase fetch logic if you want
    // For now using local fallback like in your web code
  ...
  const buildLocalSuggestions = (goalsList: LearningGoal[]): CareerSuggestion[] => {
    ...
    const careerMap: Record<string, CareerSuggestion[]> = {
      ...
      // ... add more subjects as needed (copy from your web version)
      'Computer Science': [
  ...
  const totalHours = completedSessions; // 1 hour assumption
...
const styles = StyleSheet.create({
  ...
  greeting: { fontSize: 26, fontWeight: '700', color: '#1f2937', marginBottom: 4 },
  ...
  seeAll: { color: '#f97316', fontWeight: '600' },
  ...
  sessionSubject: { fontSize: 16, fontWeight: '600', color: '#1f2937' },
  ...
  matchText: { color: '#ca8a04', fontSize: 12, fontWeight: '600' },
  suggestionDesc: { fontSize: 14, color: '#4b5563' },

  emptyText: { textAlign: 'center', color: '#6b7280', fontSize: 15, paddingVertical: 20 },
